...
public class FirstPersonController : MonoBehaviour
{
    ...
    // Internal Variables
    [field: NonSerialized] public bool isWalking = false;
    ...
    [field: NonSerialized] public bool isSprinting = false;
    ...
    // Internal Variables
    [field: NonSerialized] public bool isCrouched = false;
    ...
    private void Update()
    {
        ...
        if (enableZoom)
        {
            ...
            if (isZoomed)
            {
                playerCamera.fieldOfView = Mathf.Lerp(playerCamera.fieldOfView, zoomFOV, zoomStepTime * Time.unscaledDeltaTime);
            }
            else if (!isZoomed && !isSprinting)
            {
                playerCamera.fieldOfView = Mathf.Lerp(playerCamera.fieldOfView, fov, zoomStepTime * Time.unscaledDeltaTime);
            }
        }
        ...
        if (enableSprint)
        {
            ...
            {
                isZoomed = false;
                playerCamera.fieldOfView = Mathf.Lerp(playerCamera.fieldOfView, sprintFOV, sprintFOVStepTime * Time.unscaledDeltaTime);

                // Drain sprint remaining while sprinting
                if (!unlimitedSprint)
                {
                    sprintRemaining -= 1 * Time.unscaledDeltaTime;
            ...
            else
            {
                ...
                sprintRemaining = Mathf.Clamp(sprintRemaining += 1 * Time.unscaledDeltaTime, 0, sprintDuration);
            }
            ...
            if (isSprintCooldown)
            {
                sprintCooldown -= 1 * Time.unscaledDeltaTime;
                if (sprintCooldown <= 0)
                {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
            {
                ...
                if (velocityChange.x != 0 || velocityChange.z != 0)
                {
                    ...
                    {
                        sprintBarCG.alpha += 5 * Time.unscaledDeltaTime;
                    }
                }
            ...
            else
            {
                ...
                {
                    sprintBarCG.alpha -= 3 * Time.unscaledDeltaTime;
                }
    ...
    private void HeadBob()
    {
        if (isWalking)
        {
            ...
            if (isSprinting)
            {
                timer += Time.unscaledDeltaTime * (bobSpeed + sprintSpeed);
            }
            // Calculates HeadBob speed during crouched movement
            else if (isCrouched)
            {
                timer += Time.unscaledDeltaTime * (bobSpeed * speedReduction);
            }
